...
class Learner:
    ...
    def print_tree(self):
        print("No tree printing available for this learner")
...
class DecisionTreeLearner(Learner):
    ...
    def print_tree(self):
        if self.dt is None:
            print("Cannot print tree since none were constructed. Probably no training data.");
        elif isinstance(self.dt, str):
            print("All ==> RESULT =", self.dt)
        else:
            self.dt.display()
...
def test(learner, dataset, examples=None, verbose=0):
    ...
    if verbose:
        print(f"{correct} correct, {len(examples) - correct} wrong")
        print(f"Success rate: {100*correct / len(examples)}%")
    return correct / len(examples)

def train_and_test_all(learner, dataset, test_start, test_end, verbose=1, print_tree=0):
    """Reserve dataset.examples[start:end] for test; train on the remainder.
    Return the proportion of examples correct on the test examples."""
    
    if test_start >= test_end or test_start < 0 or test_end >= len(dataset.examples):
        print(f"Invalid bounds provided for test set: {test_start}:{test_end}")
        return None

    training_dataset = dataset.examples[:test_start] + dataset.examples[test_end:]
    testing_dataset = dataset.examples[test_start:test_end]
    
    print(f"Testing on [{test_start}:{test_end}] and the remaining {len(training_dataset)} for training.")

    testset = copy.copy(dataset)
    testset.examples = training_dataset
    learner.train(testset)

    result = test(learner, dataset, testing_dataset, verbose)
    if print_tree:
        learner.print_tree()
    return result

def is_valid_bounds(lst, start, end):
    _start = start % len(lst)
    _end = end % len(lst)
    if not (-len(lst) <= start and start < len(lst)):
        return False
    elif not (-len(lst) <= end and end < len(lst)):
        return False
    elif _start > _end:
        return False
    else:
...
def train_and_test_spec(learner, dataset, test_start, test_end, train_start, train_end, verbose=1, print_tree=0):
    """Reserve dataset.examples[start:end] for test; train on the remainder.
    Return the proportion of examples correct on the test examples."""
    examples = dataset.examples
    try:
        dataset.examples = examples[train_start-1:train_end]
        learner.dataset = dataset
        learner.train(dataset)

        result = test(learner, dataset, examples[test_start-1:test_end], verbose)
        if print_tree:
            learner.print_tree()
        return result
    finally:
