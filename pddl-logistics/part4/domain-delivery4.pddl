...
(define (domain logistics)
  ...
  (:predicates
   ...
   (large_object ?lo) (airport ?a) (airplane ?ap) (drivable ?d)
  ...
  ;; Load an object into an airplane.
  (:action load_airplane
  :parameters (?o ?ap ?a)
  :precondition (and (object ?o) (airplane ?ap) (airport ?a)
          (at ?ap ?a) (at ?o ?a))
  :effect (and (in ?o ?ap) (not (at ?o ?a))))
  ...
  ;; Drive a vehicle between two locations in the same city. Vehicle must be
  ;; of type drivable.
  (:action drive
    :parameters (?d ?l1 ?l2 ?c)
    :precondition (and (drivable ?d) (location ?l1) (location ?l2) (city ?c)
		       (at ?d ?l1) (loc ?l1 ?c) (loc ?l2 ?c))
    :effect (and (at ?d ?l2) (not (at ?d ?l1))))
  
  ;; Fly airplane between two cities.
  (:action fly
    :parameters (?ap ?a1 ?a2)
    :precondition (and (airplane ?ap) (airport ?a1) (airport ?a2)
            (at ?ap ?a1))
    :effect (and (at ?ap ?a2) (not (at ?ap ?a1))))
)
